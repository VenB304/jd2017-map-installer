...
if errorlevel 1 (
	echo.
	echo [ERROR] Python 3.12 or newer is required to install JD2017 Map Installer.
	popd
	exit /b 1
)
...
if errorlevel 1 (
	...
	powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $api='https://api.github.com/repos/git-for-windows/git/releases/latest'; $headers=@{'User-Agent'='jd2017-map-installer-setup'}; $release=Invoke-RestMethod -Headers $headers -Uri $api; $asset=$release.assets | Where-Object { $_.name -match '^MinGit-.*-64-bit\.zip$' } | Select-Object -First 1; if (-not $asset) { throw 'Could not find MinGit release.' }; $tmpZip='tools\mingit.zip'; Invoke-WebRequest -Headers $headers -Uri $asset.browser_download_url -OutFile $tmpZip; Expand-Archive -Path $tmpZip -DestinationPath 'tools\git' -Force; Remove-Item -Force $tmpZip"
	if errorlevel 1 (
...
echo.
echo [4/4] Staging AssetStudioModCLI runtime...
