...
from jd2017_installer.core.exceptions import ExtractionError, IPKExtractionError
from jd2017_installer.core.logging_config import log_exception_for_profile
from jd2017_installer.core.models import NormalizedMapData
from jd2017_installer.core.readjust_index import remove_entry
from jd2017_installer.extractors.base import BaseExtractor
from jd2017_installer.extractors.archive_ipk import (
    ArchiveIPKExtractor,
    extract_ipk,
    find_bundle_ipks,
)
from jd2017_installer.installers.game_writer import write_game_files
from jd2017_installer.parsers.normalizer import normalize

logger = logging.getLogger("jd2017.ui.workers")
...
def _ensure_jdnext_albumcoach_texture_from_coach(map_target: Path, codename: str) -> bool:
    ...
    try:
        from jd2017_installer.ui.widgets.albumcoach_dialog import create_composited_albumcoach
...
def _synthesize_jdnext_cover_if_needed(
    ...
    try:
        ...
        from jd2017_installer.installers.media_processor import synthesize_jdnext_cover_art
...
def reprocess_audio(
    ...
    from jd2017_installer.installers.media_processor import (
        convert_audio, 
        generate_intro_amb,
        extract_amb_clips,
    )
    ...
    if intro_amb_attempt_enabled:
        ...
        if map_data.music_track and map_data.music_track.markers:
            from jd2017_installer.parsers.binary_ckd import calculate_marker_preroll
        ...
        try:
            from jd2017_installer.installers.ambient_processor import _inject_intro_amb_soundset_clip
...
def pre_install_cleanup(
    ...
    from jd2017_installer.installers.sku_scene import unregister_map
...
def uninstall_map_from_game(
    ...
    from jd2017_installer.installers.sku_scene import is_registered
...
def install_map_to_game(
    ...
    if apply_gain:
        ...
        from jd2017_installer.installers.media_processor import apply_audio_gain
    ...
    if map_data.source_dir and map_data.source_dir.exists():
        ...
        from jd2017_installer.installers.media_processor import process_menu_art
    ...
    if media.moves_dir and media.moves_dir.exists():
        ...
        from jd2017_installer.installers.media_processor import copy_moves
    ...
    if source_is_jdnext and media.moves_dir and media.moves_dir.exists():
        ...
        if gesture_sources:
            ...
            if getattr(cfg, "convert_jdnext_gestures", True):
                ...
                from jd2017_installer.installers.gesture_compiler import compile_gesture_from_scratch
                compiled = 0
                ...
                from jd2017_installer.installers.gesture_compiler import copy_surrogate_as_fallback
                for gsrc in gesture_sources:
                    durango_out = durango_moves_out / gsrc.name
    ...
    if map_data.has_autodance and map_data.source_dir and map_data.source_dir.exists():
        ...
        from jd2017_installer.installers.autodance_processor import (
        ...
        from jd2017_installer.installers.autodance_processor import process_stape_file
        process_stape_file(map_data.source_dir, map_target, codename)
