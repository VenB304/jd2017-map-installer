"""Main window for the JD2017 Map Installer — PyQt6 GUI.
...
from jd2017_installer.core.logging_config import apply_log_detail, get_file_log_level
from jd2017_installer.core.install_summary import InstallSummary, build_install_summary, render_install_summary
from jd2017_installer.core.theme import load_theme_stylesheet, resolve_theme_stylesheet_path
from jd2017_installer.core.models import (
    MapMedia,
    MapSync,
    MusicTrackStructure,
    NormalizedMapData,
...
from jd2017_installer.core.readjust_index import (
    ReadjustIndexEntry,
    load_index,
    prune_stale_entries,
...
from jd2017_installer.ui.widgets import (
    ActionWidget,
    ConfigWidget,
    ModeSelectorWidget,
    PreviewWidget,
...
from jd2017_installer.ui.widgets.log_console import SUCCESS_LEVEL
from jd2017_installer.ui.workers.media_workers import (
    SyncRefinementWorker,
)
from jd2017_installer.ui.workers.pipeline_workers import (
...
logger = logging.getLogger("jd2017.ui.main_window")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # -- Window setup -----------------------------------------------------
        self.setWindowTitle("JD2017PC Map Installer")
    ...
    def _show_quickstart_if_needed(self) -> None:
        ...
        from jd2017_installer.ui.widgets.quickstart_dialog import QuickstartDialog
    ...
    def _offer_ffmpeg_install(self, missing: list[str]) -> bool:
        ...
        from jd2017_installer.ui.widgets.ffmpeg_dialog import FFmpegInstallDialog
    ...
    def _run_startup_update_check(self) -> None:
        ...
        thread = QThread(self)
        from jd2017_installer.ui.widgets.update_dialog import UpdateResultDialog
    ...
    def _collect_source_target_issues(self) -> list[str]:
        ...
        from jd2017_installer.ui.widgets.mode_selector import (
            MODE_FETCH,
            MODE_JDNEXT,
            MODE_FETCH_JDLO,
            MODE_HTML,
    ...
    def _on_preflight(self) -> None:
        ...
        from jd2017_installer.ui.widgets.mode_selector import MODE_FETCH, MODE_JDNEXT, MODE_FETCH_JDLO
        source_state = self._mode_selector.get_current_state()
    ...
    def _on_settings(self) -> None:
        from jd2017_installer.ui.widgets.settings_dialog import SettingsDialog
    ...
    def _load_readjust_from_folder(self, folder: str) -> None:
        from jd2017_installer.parsers.normalizer import normalize
    ...
    def _on_install_requested(self) -> None:
        ...
        from jd2017_installer.ui.widgets.mode_selector import MODE_FETCH, MODE_JDNEXT, MODE_FETCH_JDLO, MODE_LABELS
        source_state = self._mode_selector.get_current_state()
        mode_index = int(source_state.get("mode_index", -1))
        ...
        from jd2017_installer.ui.widgets.mode_selector import MODE_BATCH
        ...
        # Bundle IPK support
        from jd2017_installer.ui.widgets.mode_selector import MODE_IPK
        if mode_index == MODE_IPK and Path(self._current_target).is_file():
            from jd2017_installer.extractors.archive_ipk import validate_ipk_magic
            try:
            ...
            if len(maps_found) > 1:
                from jd2017_installer.ui.widgets.bundle_dialog import BundleSelectDialog
        ...
        from jd2017_installer.extractors.manual_extractor import ManualExtractor
        ...
        def on_finished(success: bool, msg: str) -> None:
            ...
            if success:
                ...
                self.append_log("Install completed successfully!")
        ...
        worker.signals.finished.connect(lambda ok: thread.quit())
        worker.signals.finished.connect(lambda ok: worker.deleteLater())
        thread.finished.connect(thread.deleteLater)
    ...
    def _check_metadata(self, map_data: NormalizedMapData) -> None:
        ...
        from jd2017_installer.ui.widgets.metadata_dialog import MetadataCorrectionDialog
    ...
    def _apply_albumcoach_customization(self, map_data: NormalizedMapData) -> None:
        ...
        if behavior == "always_customize":
            ...
            from jd2017_installer.ui.widgets.albumcoach_dialog import AlbumCoachCustomizerDialog
        ...
        if result_image is not None:
            ...
            import tempfile
            tmp_dir = Path(tempfile.mkdtemp(prefix="jd2017_albumcoach_"))
    ...
    def _show_install_summary_popup(self, summaries: list[InstallSummary]) -> None:
        ...
        from jd2017_installer.ui.widgets.installation_summary_dialog import InstallationSummaryDialog
    ...
    def _on_apply_offset(self, audio_ms: float, video_ms: float) -> None:
        ...
        from jd2017_installer.ui.workers.pipeline_workers import ApplyAndFinishWorker
    ...
    def _resolve_extractor(self):
        ...
        if idx == MODE_IPK:
            from jd2017_installer.extractors.archive_ipk import ArchiveIPKExtractor
        ...
        if idx == MODE_FETCH_JDLO:
            from jd2017_installer.extractors.jdlo_extractor import JDLOExtractor
        ...
        if idx in (MODE_FETCH, MODE_JDNEXT):
            from jd2017_installer.extractors.web_playwright import WebPlaywrightExtractor
        ...
        if idx == MODE_HTML_JDLO:
            from jd2017_installer.extractors.jdlo_extractor import JDLOOfflineExtractor
    ...
    def _resolve_ipk_bundle_inputs(
        ...
        from jd2017_installer.extractors.archive_ipk import find_bundle_ipks
    ...
    def _start_batch_install(
        ...
        from jd2017_installer.ui.workers.pipeline_workers import BatchInstallWorker
    ...
    def _start_file_logging(self, current_target: str, mode_name: str = "", action: str = "install") -> None:
        ...
        for log_path in log_paths:
            ...
            logging.getLogger("jd2017").addHandler(handler)
    ...
    def _stop_file_logging(self) -> None:
        ...
        if self._file_logger_handlers:
            for handler in self._file_logger_handlers:
                logging.getLogger("jd2017").removeHandler(handler)
                handler.close()
